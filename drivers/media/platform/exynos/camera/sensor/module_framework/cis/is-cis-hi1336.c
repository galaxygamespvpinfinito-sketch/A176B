...
int sensor_hi1336_set_flip_register(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	case SENSOR_FLIP_X:
		ret = cis->ixc_ops->write16(cis->client, 0x0202, 0x0100);
		if (ret < 0)
			goto p_err;
	case SENSOR_FLIP_Y:
		...
		break;
	case SENSOR_FLIP_XY:
		ret = cis->ixc_ops->write16(cis->client, 0x0202, 0x0300);
		if (ret < 0)
			goto p_err;
		break;
	default:
...
int sensor_hi1336_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_hi1336_set_flip_register(subdev);
