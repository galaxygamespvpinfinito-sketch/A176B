...
int sensor_jn1_set_flip_register(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	case SENSOR_FLIP_X:
		...
		if (ret < 0)
			goto p_err;
		ret = cis->ixc_ops->write8(cis->client, 0x0101, 0x01);
		if (ret < 0)
			goto p_err;
		break;
	case SENSOR_FLIP_Y:
		...
		if (ret < 0)
			goto p_err;
		ret = cis->ixc_ops->write8(cis->client, 0x0101, 0x02);
		if (ret < 0)
			goto p_err;
		break;
	case SENSOR_FLIP_XY:
...
int sensor_jn1_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_jn1_set_flip_register(subdev);
