...
int sensor_sc201_set_flip_register(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	struct i2c_client *client;
	...
	case SENSOR_FLIP_X:
		ret = cis->ixc_ops->write8(client, 0x3221, 0x06);
		if (ret < 0)
			goto p_err;
	case SENSOR_FLIP_Y:
		ret = cis->ixc_ops->write8(client, 0x3221, 0x60);
		if (ret < 0)
			goto p_err;
	case SENSOR_FLIP_XY:
...
int sensor_sc201_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	/* It should be called here not in global_setting */
	ret = sensor_sc201_set_flip_register(subdev);
