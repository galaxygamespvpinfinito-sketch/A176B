...
#else
int is_mcu_open_fw(struct v4l2_subdev *subdev, char *name, ulong *buf_size)
{
	int ret = 0;
	ulong size = 0;
	const struct firmware *fw_blob = NULL;
	...
	fw_requested = 0;
	size = bin.size;
	info("mcu start read sdcard, file path %s%s, size %lu Bytes\n", IS_OIS_SDCARD_PATH, name, size);

	ois_fw_buf = vmalloc(size);
	if (!ois_fw_buf) {
		err("failed to allocate memory");
		ret = -ENOMEM;
		goto p_err;
	...
	memcpy(ois_fw_buf, (char *)bin.data, bin.size);

	memcpy(&mcu->vdrinfo_bin[0], (u8 *)(ois_fw_buf + 0x807C), sizeof(mcu->vdrinfo_bin));
	mcu->hw_bin[0] = (u8)*(ois_fw_buf + 0x80FB);
	mcu->hw_bin[1] = (u8)*(ois_fw_buf + 0x80FA);
	mcu->hw_bin[2] = (u8)*(ois_fw_buf + 0x80F9);
	mcu->hw_bin[3] = (u8)*(ois_fw_buf + 0x80F8);
	memcpy(ois_pinfo.header_ver, mcu->hw_bin, 4);
	memcpy(&ois_pinfo.header_ver[4], (char *)(ois_fw_buf + 0x807C), 4);
...
request_fw:
	if (fw_requested) {
		...
		memcpy((void *)ois_fw_buf, fw_blob->data, size);
		memcpy(&mcu->vdrinfo_bin[0], (u8 *)(ois_fw_buf + 0x807C), sizeof(mcu->vdrinfo_bin));
		mcu->hw_bin[0] = (u8)*(ois_fw_buf + 0x80FB);
		mcu->hw_bin[1] = (u8)*(ois_fw_buf + 0x80FA);
		mcu->hw_bin[2] = (u8)*(ois_fw_buf + 0x80F9);
...
void is_mcu_fw_update(struct is_core *core)
{
	...
	ulong size = 0;
	u8 SendData[256] = {0, };
#ifdef UPDATE_OIS_FIRMWARE_IF_NECESSARY
	int vdrinfo_bin = 0;
	int vdrinfo_mcu = 0;
#endif
	struct i2c_client *client = NULL;
	...
	ret = is_mcu_open_fw(subdev, FIMC_MCU_FW_NAME, &size);
	if (ret < 0) {
...
int is_set_ois_mode_mcu(struct v4l2_subdev *subdev, int mode)
{
	...
	switch(mode) {
		...
		case OPTICAL_STABILIZATION_MODE_VDIS_ASR:
			is_ois_i2c_write(client, 0x0002, 0x15);
			is_ois_i2c_write(client, 0x0000, 0x01);
			break;
...
void is_ois_shift_mcu(struct v4l2_subdev *subdev, int16_t *shiftValue)
{
	int i = 0;
	int j = 0;
	struct is_ois *ois = NULL;
...
int ois_mcu_get_hall_data(struct v4l2_subdev *subdev, struct is_ois_hall_data *halldata)
{
	...
	I2C_MUTEX_LOCK(ois->i2c_lock);
#ifdef OIS_ANGLE_SUPPORT
	ret = is_ois_i2c_read_multi(client, 0x0560, &arr[0], 20);
#else
...
#ifdef OIS_ANGLE_SUPPORT
	halldata->X_Angle[0] = hall_data[0];
	halldata->Y_Angle[0] = hall_data[1];
	halldata->X_Angle[1] = hall_data[2];
	halldata->Y_Angle[1] = hall_data[3];
	halldata->X_Angle[2] = hall_data[4];
	halldata->Y_Angle[2] = hall_data[5];
	halldata->X_Angle[3] = hall_data[6];
	halldata->Y_Angle[3] = hall_data[7];
...
void ois_mcu_get_hall_position_mcu(struct is_core *core, u16 *targetPos, u16 *hallPos)
{
	...
	/* set centering mode */
	is_ois_i2c_write(client, 0x02, 0x05);


	/* enable position data read */
	is_ois_i2c_write(client, 0x80, 0x01);

	msleep(150);

	is_ois_i2c_read_multi(client, 0x86, pos_temp, 4);
	pos = (pos_temp[1] << 8) | pos_temp[0];
	targetPos[0] = pos;

	is_ois_i2c_read_multi(client, 0x88, pos_temp, 4);
	pos = (pos_temp[1] << 8) | pos_temp[0];
	targetPos[1] = pos;

	is_ois_i2c_read_multi(client, 0x8E, pos_temp, 4);
	pos = (pos_temp[1] << 8) | pos_temp[0];
	hallPos[0] = pos;

	is_ois_i2c_read_multi(client, 0x90, pos_temp, 4);
	pos = (pos_temp[1] << 8) | pos_temp[0];
	hallPos[1] = pos;

	/* disable position data read */
	is_ois_i2c_write(client, 0x80, 0x00);
