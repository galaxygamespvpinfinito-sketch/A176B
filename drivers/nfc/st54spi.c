...
#define ST54SPI_IOC_WR_RESET _IOW(SPI_IOC_MAGIC, 0xE, __s32)
...
struct st54spi_data {
	...
	int cs_gpio;
	u32 read_1byte_cnt; // counter for read 1 byte commands
};
				...
				loff_t *f_pos)
{
	...
	if (count == 1 && (st54spi->rx_buffer[0] == 0 || st54spi->rx_buffer[0] == 0xFF)) {
		st54spi->read_1byte_cnt++;
	} else {
		if (st54spi->read_1byte_cnt)
			NFC_LOG_INFO("ese r%zu t%u\n", count, st54spi->read_1byte_cnt);
		else
			NFC_LOG_INFO("ese r%zu\n", count);
		st54spi->read_1byte_cnt = 0;
	}
				...
				size_t count, loff_t *f_pos)
{
	...
	if (st54spi->read_1byte_cnt)
		NFC_LOG_INFO("ese w%zu 1r:%u\n", count, st54spi->read_1byte_cnt);
	else
		NFC_LOG_INFO("ese w%zu\n", count);
				...
				unsigned long arg)
{
	...
	case ST54SPI_IOC_WR_POWER:
		...
		break;
	case ST54SPI_IOC_WR_RESET:
		if (GPIO_IS_VALID(st54spi->ap_ese_reset_gpio)) {
			gpio_direction_output(st54spi->ap_ese_reset_gpio, 0);
			usleep_range(1500, 1600);
			gpio_direction_input(st54spi->ap_ese_reset_gpio);//high Z
			NFC_LOG_INFO("%s AP cold reset\n", __func__);
		} else {
			NFC_LOG_INFO("%s ap_ese_reset_gpio not set\n", __func__);
		}
		break;
	default:
...
{
	if (debug_enabled)
		NFC_LOG_INFO("st54spi compat ioctl cmd 0x%X\n", cmd);
