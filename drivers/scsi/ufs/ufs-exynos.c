...
static void exynos_ufs_select_vreg(struct ufs_hba *hba)
{
#ifdef CONFIG_SOC_S5E8535
	int mem_sel = main_pmic_mem_sel_status();
	struct device *dev = hba->dev;
	struct ufs_vreg_info *info = &hba->vreg_info;
	struct ufs_vreg **vreg;
	const char *msg[2] = {"VCCQ", "VCCQ2"};

	if (!info->vccq || !info->vccq2)
		return;

	vreg = (mem_sel &= 1) ? &info->vccq2 : &info->vccq;
	if (!*vreg) {
		pr_info("%s: failed to get %s\n",
				__func__, msg[mem_sel]);
		return;
	}
	/* MEM_SEL: 1 -> LP5, LDO31M
	 * MEM_SEL: 0 -> LP4, LDO30M
	 */
	dev_info(dev, "%s: vreg = %s, mem_sel = %d\n",
		 __func__, (*vreg)->name, mem_sel);
					...
					enum ufs_notify_change_status notify)
{
	...
	if (ufs->power_init) {
		exynos_ufs_select_vreg(hba);
		ufs->power_init = false;
	}
